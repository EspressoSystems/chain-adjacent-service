...
mod tests {
    ...
    fn empty_message_with_metadata() -> MessageWithMetadata {
    ...
    async fn test_broadcaster_messages_removed_on_confirmation() {
        ...
        b.broadcast_single(empty_message_with_metadata(), 1, None, vec![])
            .expect("broadcast 1");
        wait_until_count(&b, 1, "after 1 message").await;

        b.broadcast_single(empty_message_with_metadata(), 2, None, vec![])
            .expect("broadcast 2");
        wait_until_count(&b, 2, "after 2 messages").await;

        b.broadcast_single(empty_message_with_metadata(), 3, None, vec![])
            .expect("broadcast 3");
        wait_until_count(&b, 3, "after 3 messages").await;

        b.broadcast_single(empty_message_with_metadata(), 4, None, vec![])
            .expect("broadcast 4");
        wait_until_count(&b, 4, "after 4 messages").await;

        b.broadcast_single(empty_message_with_metadata(), 5, None, vec![])
            .expect("broadcast 5");
        wait_until_count(&b, 5, "after 5 messages").await;

        b.broadcast_single(empty_message_with_metadata(), 6, None, vec![])
            .expect("broadcast 6");
        ...
        b.confirm(5);
        b.broadcast_single(empty_message_with_metadata(), 7, None, vec![])
            .expect("broadcast 7");
    ...
    async fn test_broadcast_feed_messages_appended_to_backlog() {
        ...
        let msgs: Vec<BroadcastFeedMessage> = (1..=3)
            .map(|i| BroadcastFeedMessage {
                sequence_number: i,
                message: empty_message_with_metadata(),
                block_hash: None,
                signature: vec![],
                block_metadata: vec![],
                cumulative_sum_msg_size: 0,
    ...
    async fn test_new_broadcast_feed_message_with_signer() {
        ...
        let bfm = b
            .new_broadcast_feed_message(empty_message_with_metadata(), 1, None, vec![])
    ...
    async fn test_new_broadcast_feed_message_without_signer() {
        ...
        let bfm = b
            .new_broadcast_feed_message(empty_message_with_metadata(), 5, None, vec![])
            .expect("create feed message");
    ...
    fn test_compute_message_hash_deterministic() {
        let msg = empty_message_with_metadata();
